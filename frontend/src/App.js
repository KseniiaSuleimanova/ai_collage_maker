    ...
    const handleGetCollage = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.filename) {
                    const imagePath = `http://127.0.0.1:5000/static/${data.filename}`;
                    setImageUrl(imagePath); // Update imageUrl to display the image
                    alert("Collage created successfully");
                } else {
                    alert("Collage created, but no filename returned");
                }
            } else {
            ...
            <div style={{textAlign: 'center', marginTop: '50px'}}>
                ...
                {imageUrl && (
                    <img
                        src={imageUrl}
                        alt="From Backend"
                        style={{width: '900px', height: '900px', objectFit: 'cover', border: '1px solid #ccc'}}
                    />
                )}
            </div>
        </div>
